...
const ManageReservationContainer: React.FC = () => {
    ...
    .filter(reservation => {
      ...
      const matchesSearch = `${reservation.firstName} ${reservation.lastName}`
        .toLowerCase()
      ...
      if (dateFilter.type && dateFilter.value) {
        const reservationDate = dayjs(
          dateFilter.type === 'checkIn' ? reservation.checkIndate :
          dateFilter.type === 'checkOut' ? reservation.checkOutdate :
          ...
          <table className="min-w-full text-black">
            <thead className="bg-gray-200 text-gray-500 font-bold text-sm">
              <tr>
                ...
                >
                  Khách hàng {sortField === "firstName" && (sortOrder === "asc" ? "▲" : "▼")}
                </th>
                <th
                  className="px-4 py-3 text-left cursor-pointer"
                  onClick={() => handleSort("checkIndate")}
                >
                  Check-in {sortField === "checkIndate" && (sortOrder === "asc" ? "▲" : "▼")}
                </th>
                <th
                  className="px-4 py-3 text-left cursor-pointer"
                  onClick={() => handleSort("checkOutdate")}
                >
                  Check-out {sortField === "checkOutdate" && (sortOrder === "asc" ? "▲" : "▼")}
                </th>
                <th
                  className="px-4 py-3 text-left cursor-pointer"
                ...
                >
                  Tổng tiền {sortField === "totalPrice" && (sortOrder === "asc" ? "▲" : "▼")}
                </th>
                <th
                  className="px-4 py-3 text-left cursor-pointer"
                  onClick={() => handleSort("createdAt")}
                >
                  Ngày đặt {sortField === "createdAt" && (sortOrder === "asc" ? "▲" : "▼")}
                </th>
            ...
            <tbody className="text-sm font-semibold">
              {currentRows.map((reservation) => (
                ...
                >
                  ...
                  <td className="px-4 py-5">{reservation.totalPrice.toLocaleString()} VND</td>
                  <td className="px-4 py-5">
