...
const ListRoomContainer: React.FC<IProps> = ({
  ...
  const { setPropertyId, setRoomId, setStartDate, setEndDate, startDate, endDate } =
    useCheckoutContext();
  const router = useRouter();
  const [rooms, setRooms] = useState<IRoom[]>([]);
  const [listRoomAvailability, setListRoomAvailability] = useState<{id: string, status: boolean}[]>([]);
  ...
  useEffect(() => {
    const fetchDataRoomAvailability = async() => {
      const response = await apisRoomAvailability.checkRoomAvailabilityByPropertyId({
        propertyId,
  ...
  return (
    <div>
      {rooms.map((item: IRoom) => (
        <div key={item.id} className="shadow-md bg-white border-gray-300 rounded-md p-4 mt-2">
        <h2 className="mb-4 font-semibold text-lg">{item.name}</h2>
        <div className="flex gap-4">
          <div className="w-[30%]">
            <img alt="anh" className="rounded-md w-full h-[140px] object-cover" src={item.images[0]?.image}></img>
            ...
            <div className="flex">
              <div className="w-[70%]">
                ...
                {item.summaries.map((summary: ISummary) => (
                  <p key={summary.id} className="flex items-center gap-2 mt-2">
            ...
            <div className="flex justify-end mt-4">
              <div>
                <div className="flex items-center">
                  ...
                  <button
                    onClick={() => {
                        if (listRoomAvailability.some((subItem) => (subItem.id === item.id && subItem.status === true))) {
                    ...
                    className={clsx(
                        ...
                        {"opacity-50 cursor-not-allowed": listRoomAvailability.some((subItem) => (subItem.id === item.id && subItem.status === false)
                      )}
                    )}
                  >
                    Đặt ngay
                ...
                {listRoomAvailability.some((subItem) => (subItem.id === item.id && subItem.status === false)) &&
                  <p className="text-[11px] font-semibold text-red-500 flex justify-end mt-1">Phòng đã được đặt hết</p>
                }
              </div>
            </div>
          </div>
