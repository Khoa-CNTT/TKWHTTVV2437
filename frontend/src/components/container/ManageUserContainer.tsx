...
import { showSuccessAlert, showErrorAlert, showConfirmAlert } from "@/helper/Alert";
...
const ManageUserContainer: React.FC = () => {
    ...
    const handleLockUser = async (userId: string) => {
        try {
            const confirmLock = await showConfirmAlert("Bạn có chắc chắn muốn khóa tài khoản này?");
            if (confirmLock) {
    ...
    }> = ({ users, onDeleteUser, onLockUser, onSelectUser,/*onApproveUser,*/ deletingUserId }) => {
        ...
        return (
            ...
            <thead className="bg-gray-200 text-gray-500 font-bold text-sm">
              <tr>
                ...
                >
                  Họ và Tên {sortField === "firstName" && (sortOrder === "asc" ? "▲" : "▼")}
                </th>
                <th
                  className="px-4 py-3 text-left cursor-pointer"
                ...
                >
                  Vai trò {sortField === "role" && (sortOrder === "asc" ? "▲" : "▼")}
                </th>
    ...
    const DetailUser: React.FC<{ user: IUser; onClose: () => void; onUpdate: () => void; }> = ({
      user,
      onClose,
      onUpdate,
    }) => {
          ...
          <div className="w-2/5 bg-white h-full p-6 shadow-lg flex flex-col justify-between transform transition-transform duration-300 translate-x-0">
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block font-bold text-sm">Số điện thoại:</label>
                  <input
                    type="text"
                    name="phone"
                ...
                <div>
                  ...
                  <select
                    name="gender"
                    value={editingUser ? editingUser.gender || '' : user.gender || ''}
                    onChange={handleEditChange}
                    disabled={!isEditing}
                    className="border rounded px-2 py-1 w-full text-sm"
                  >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    name="emergencyPhone"
                    value={editingUser ? editingUser.emergencyPhone || '' : user.emergencyPhone || ''}
                    onChange={handleEditChange}
                    disabled={!isEditing}
                    className="border rounded px-2 py-1 w-full text-sm"
                  />
                ...
                <div className="col-span-2">
                  ...
                  <input
                    type="text"
                    name="address"
                    value={editingUser ? editingUser.address || '' : user.address || ''}
                    onChange={handleEditChange}
                    disabled={!isEditing}
                    className="border rounded px-2 py-1 w-full text-sm"
                  />
