...
const ConfirmApprove = ({ handleCloseAppove }: IProps) => {
  ...
  return (
    <div
      className="fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-30 flex justify-center items-center z-50 h-screen"
      onClick={() => {
    ...
    >
      ...
      >
        ...
        >
          Yêu cầu {reservation?.statusUser === "created" ? "đặt" : "hủy"} phòng
        </h1>
        ...
        <div>
          Loại phòng: <span className="font-bold">{reservation?.nameRoom}</span>
        </div>
        <div>
